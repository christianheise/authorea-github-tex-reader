...
function latex2html($line) {
  ...
  $line = preg_replace('/\\\\url\{(.*)\}/U','<a href="\\1" target="_blank">\\1</a>', $line);
...
//     echo $this->getKey();
  ...
  */
  ...
  function getBibLink($iconurl=NULL) {
    ...
    $href = 'href="../includes/'.$this->getURL().'"';
...
function print_header_layout() {
  ...
  echo '<' . get_HTML_tag_for_layout() . ' class="result table table-responsive">'."\n";
}
...
function DefaultBibliographyStyle(&$bibentry) {
  ...
  if ($bibentry->hasField('url')) $title = ' <a'.(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'').' target="_blank" href="'.$bibentry->getField('url').'">'.$title.'</a>';
...
class SimpleDisplay  {
  ...
  function display() {
    ...
    if ($this->headingLevel == BIBTEXBROWSER_HTMLHEADINGLEVEL) {
      ...
      if (count($this->entries) == 1) {
        echo count ($this->entries).' '.__('Ergebnisse');
  ...
  function sectionHeader($bib, $pred) {
    switch(BIBTEXBROWSER_LAYOUT) {
      case 'table':
      if($bib->getYear()!=""){
        return '<tr><td colspan="2" class="'.$this->headerCSS.'" styLe="padding-top:40px; border-top:0;">'.$bib->getYear().'</td></tr>'."\n";
         }
         else {
         return '<tr><td colspan="2" class="'.$this->headerCSS.'" styLe="padding-top:40px; border-top:0;">unzugeordnet</td></tr>'."\n";
...
</head><body><div class="container" style="padding-top:10px;">
  <p class="bg-warning" style="padding:5px;"><b>Hinweis:</b> Der Eintrag existiert (noch) nicht.</p><br>
 <a href="javascript:history.back()" class="btn btn-default btn-xs" role="button">zur&uuml;ck zum Text</a><p>&nbsp;</p>
 </div></body></html>
  <?php
  exit;
}
...
class AcademicDisplay  {
  ...
  function search2html($query, $title) {
    ...
    if (count($entries)>0) {
      echo "\n".'<div class="sheader" style="">'.$title.'</div>'."\n";
    }
...
class BibEntryDisplay {
  ...
  /** returns the title */
  function getTitle() {
    return '<h3>'.$this->bib->getTitle().' <small>(bibtex)</small></h3>';
  }
  ...
  function displayOnSteroids() {
      ...
      if ($this->bib->hasField('url')) {
        $download = '<div  style="display:inline;padding-top:5px;" class="bibentry-document-link" style="padding-top:10px;"><a href="'.$this->bib->getField('url').'" class="btn btn-default btn-xs" role="button" target="_blank">Text/PDF anzeigen</a></div>';
      }
      
       if ($this->bib->hasField("doi")) {
        $download .= ' <div style="display:inline;padding-top:5px;"><a href="http://dx.doi.org/'.$this->bib->getField("doi").'" class="btn btn-default btn-xs" role="button" target="_blank">DOI: '.$this->bib->getField("doi").'</a></div>';
      }
  ...
  function display() {
    ...
    echo '<a href="javascript:history.back()" class="btn btn-default btn-xs" role="button">zur&uuml;ck zum Text</a><p>&nbsp;</p></div>';
  }
...
<body><div style="padding-top:0px;" class="container">
<?php
if (method_exists($content, 'getTitle')) {
